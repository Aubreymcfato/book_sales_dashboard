# app.py → VERSIONE FINALE 2025-12-05 – TUTTO FUNZIONA
...
if not os.path.exists(MASTER_PATH):
    ...
    for f in files:
        ...
        try:
            ...
            # collana
            coll_col = next((c for c in ["collana","collection","series","collection/series"] if c in df.columns), None)
            if coll_col:
                df = df.rename(columns={coll_col: "collana"})
    ...
    st.success("Database master creato!")
    ...
    if selected_week == "Tutti" and any(filters.values()):
        ...
        for w in week_options[1:]:
            ...
            if filters.get("title"):
                ...
                label = "Editore"
            elif filters.get("collana"):
                temp = temp[temp["collana"].isin(filters["collana"])]
                label = "Collana"
            else:
                continue
            units = int(temp["units"].sum())
            trend.append({"Settimana": w, "Unità": units, "Filtro": label})
...
# ===================================================================
# TAB ANALISI ADELPHI – HEATMAP + VENDITE NEL TOOLTIP
# ===================================================================
with tab_adelphi:
    st.header("Analisi Variazioni Settimanali – Adelphi")

    adelphi = df_all = df_all[df_all["publisher"].str.contains("Adelphi", case=False, na=False)].copy()
    if adelphi_all.empty:
        st.info("Nessun dato Adelphi.")
        st.stop()

    adelphi_all["units"] = pd.to_numeric(adelphi_all["units"], errors="coerce").fillna(0)

    # Applica filtri
    for col in ["title", "collana"]:
        if filters.get(col):
            adelphi_all = adelphi_all[adelphi_all[col].isin(filters[col])]

    grp = ["title", "week"]
    if "collana" in adelphi_all.columns:
        grp.insert(1,"collana")
    adelphi = adelphi_all.groupby(grp)["units"].sum().reset_index()
    ...
    # Aggiungiamo le vendite assolute per il tooltip
    ...
    st.dataframe(
        adelphi[["title","collana","week","units","Diff_%"]].sort_values(["title","week"]),
        use_container_width=True
    )
